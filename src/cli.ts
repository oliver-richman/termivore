			...
			.action(({args}) => {
				...
				if (args.command){
					...
					if (!commandsToDisplay.length) {
						log(`No command with this name exists, run ${this.rootCommand} help to see all available commands`).red().print();
						return;
					}
				}
				...
				log(`${this.rootCommand}:`).bold().print();
					log('Usage:').append(log(`${this.rootCommand} [command] [arguments] [options]`).italic()).print();
					log('').print();
				...
				for (const command of commandsToDisplay) {
					...
					if (!args.command) {
						log('-').append(log(`${command.name}${formattedArgs ? ` ${formattedArgs}` : ''}${formattedOpts ? ` ${formattedOpts}` : ''}`).hex('#8590c7')).print();
					} else {
						log(`${command.name}${formattedArgs ? ` ${formattedArgs}` : ''}${formattedOpts ? ` ${formattedOpts}` : ''}`).hex('#8590c7').print();
					}
